...
Core tests take longer than any other KickAssCoin tests, due to the high amount of computational work involved in validating core components.
...
To run only KickAssCoin's core tests (after building):
...
To run only KickAssCoin's crypto tests (after building):
...
```bash
kickasscoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `kickasscoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only KickAssCoin's hash tests (after building):
...
To run only KickAssCoin's performance tests (after building):
...
To run only KickAssCoin's unit tests (after building):
